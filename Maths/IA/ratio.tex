...
\]
where $A_n$ and $B_n$ are constants. Our initial prior is achieved by setting $A_0=1$ and $B_0=0$. The example of 1 like shows that $A_1 = 2$ and $B_1 = 1$, thus I hypothesize that $A_n = n+1$ and $B_n = n$ (for proof, see appendix \ref{eq:nlikeproof}).
...
Therefore statistical mean $\mu_n$ for the $n$th like's posterior function is:
\begin{align*}
...
\begin{figure}[H]
    \paragraph{Proof} I will prove the N-Likes Generalization statement for $n\in \mathbb{N}$ through mathematical induction.
    ...
    Therefore by the principle of mathematical induction, because the case holds for $n=1$, and if $n=k$ is held, $n=k+1$ will be held, so the case holds for all $n\in\mathbb{N}$.
