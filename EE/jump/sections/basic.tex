...
To find the value of the constant $w$ that best fulfills this inequality, I need a Quantitatively measure --- I called it the error function --- on how well this inequality is held through the time domain. Ideally the function should only include the failures of fulfilling the inequality, where RHS is larger than one, so I chose an integral from $t=0$ to $t=t_f$ on the only positive values of the RHS. This is done with a custom piecewise function $H(x)$ such that
\[
 H(x) = \begin{cases}
         x & x > 0\\
         0 & \text{otherwise}
...
as to only extract the possible values. Therefore the error function $R(w)$ for each constant $w$ is defined to be the integral of the positive values of the RHS of the inequality over the jump.
\begin{figure}[H]
 \centering
 \[
  R(w) = \int_0^{t_f} H\left(\left(-\frac{k}{w} \cos(wt) + \frac{k}{w}\right) \sin(wt) + \left(\frac{k}{w} \sin(wt) - 250\right) \cos(wt) - 250\right) \, dt
 \]
...
\begin{figure}[H]
 ...
 \caption{The graph of the error function}
 \label{fig:2error}
\end{figure}

Figure \ref{fig:2error} shows the graph of the error function in equation \ref{eq:2error}. Once again the graph is symmetrical because of the rotational invariant of the problem.
